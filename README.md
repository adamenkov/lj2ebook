...
_Texpad can be super-slow even on high-end machines, because of enormous amount of information (indirectly) contained in the project file alexandrov-g.tex.  It's probably cause by Texpad's efforts to come up with the hierarchical structure of the project.  So here's a workaround: in file build_book_tex.pl, set both post_lower_bound = post_upper_bound to the same value, e.g. 757.  If you run it, you'll end up with a very small alexandrov-g.tex, which is totally digestible by Texpad._
